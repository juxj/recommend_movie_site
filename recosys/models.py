...
class MovieManager(models.Manager):
    def create_movie(self, index, name):
        movie = self.create(unique_id=index, name=name,
                            date=timezone.now())
        return movie
...
class Movie(models.Model):
    ...
    avg_rates = models.FloatField(default=0)
...
class UserManager(models.Manager):
    def create_user(self, index, name):
        user = self.create(unique_id=index, name=name)
        return user
...
class User(models.Model):
    ...
    def top_suggest(self):
        return self.suggestrate_set.order_by('rate').reverse()[:10]
